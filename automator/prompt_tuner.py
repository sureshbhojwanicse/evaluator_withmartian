...
```Final Verdict: "[[C]]"``` if they are similar in quality.
    ...
    user_prompt = dspy.InputField(desc="The prompt which contain user conversation and both candidate and reference answer")
...
class Judgementor(dspy.Module):
    ...
    def __init__(self, model_id: str="gpt-3.5-turbo-0125"):
        ...
        global llm
        llm = OpenAI(model=model_id, max_tokens=2000, api_key=os.environ.get(OAI_API_KEY_ID))
    ...
    def forward(self, user_prompt):
        ...
        global llm
        prompt_used = llm.history[-1]["prompt"] #inspect_history(n=1).split("Output")[0]
        response = llm.history[-1]["response"]

        # Return the prediction details
        return dspy.Prediction(prompt=user_prompt, judgement=prediction.output, updated_prompt=prompt_used, response=response)
...
class PromptTuner:
    ...
    def validate_judgements(self, example: dspy.Example, pred: dspy.Prediction, trace=None):
        """
    ...
    def prepare_training_data(self):
        ...
        for _, row in enrich_judgments_df.iterrows():
            ...
            reference_answer = Answer.get_answer_by_id(judgement.comparison_answers[0]).content
            ...
            example = dspy.Example(user_prompt=prompt, output=output).with_inputs("user_prompt")
    ...
    def train(self, save_path):
        ...
        teleprompter = BootstrapFewShot(metric=self.validate_judgements, max_bootstrapped_demos=4, max_labeled_demos=3)
        teleprompter2 = LabeledFewShot(k=2)

        # Compile the DSPy module
        self.compiled_class = teleprompter.compile(Judgementor(model_id=self.model_id), trainset=self.train_data)
    ...
    def eval(self):
        ...
        evaluator = Evaluate(devset=self.eval_data, num_threads=1, display_progress=True, display_table=5)
