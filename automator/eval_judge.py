...
from llm_judge.judges.concrete_judges.dynamic_few_shot_judge import DynamicFewShotJudge # expecting voyager API key when importing because of the pkg init but solved by putting client creation into class
...
class EvalJudge:
    ...
    def __init__(
        ...
        self.ideal_judgment_df["ideal_judgment_score"] = self.ideal_judgment_df["score"].copy()
    ...
    def evaluate_judge_prompt(
        ...
        merged_results["exp_judge_score"] = exp_judge_results["score"].apply(ast.literal_eval)
        merged_results["ideal_judge_score"] = merged_results["ideal_judgment_score"].apply(ast.literal_eval)
